...
use crate::spi::{spi_read, spi_write, spi_debug};
...
pub async fn run(
    ...
    if verbose { println!("Server started"); }
    iniciar_gpiod();

    loop {
        let (socket, addr) = listener.accept().await.unwrap();
        if verbose { println!("Conection from: {:?}", addr); }
...
async fn handle_connection(
    ...
    loop {
        ...
        if n_bytes != 4 { 
            if n_bytes == 0 { break; }
            if verbose { println!("Number of bytes is not 4, received {} bytes", n_bytes); }
            continue; 
        }
...
fn iniciar_gpiod(){
    ...
    if !child.stdout.is_empty() { return; }
